...
import { type Node, type Document } from "./nodes.ts";

// Returned by a visitor to control the walk; anything else (incl. `undefined`)
// descends as usual.
const VISIT_BREAK: unique symbol = Symbol("visit:break"); // stop the whole traversal
const VISIT_SKIP: unique symbol = Symbol("visit:skip"); // don't descend into this node's children
  ...
  switch (node.kind) {
    case "sequence":
      for (const item of node.items) {
        if (visitNode(item, visitor, { depth, parent: node, isKey: false }))
          return true;
      }
      break;
    case "mapping":
      for (const { key, value } of node.items) {
        if (visitNode(key, visitor, { depth, parent: node, isKey: true }))
          return true;
        if (visitNode(value, visitor, { depth, parent: node, isKey: false }))
          return true;
      }
      break;
  }
...
function visit(documents: Document[], visitor: Visitor): void {
  for (const doc of documents) {
    if (
      doc.contents &&
      visitNode(doc.contents, visitor, { depth: 0, parent: null, isKey: false })
    )
      return;
  }
}

export { visit, VISIT_BREAK, VISIT_SKIP, type Visitor, type VisitContext };
