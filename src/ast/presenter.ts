...
function resolveImplicitTag(state: PresenterState, str: string) {
  for (
    ...
    if (
      tagDefinition.resolve(str, false, tagDefinition.tagName) !== NOT_RESOLVED
    ) {
...
function chooseScalarStyle(
  ...
  string: string,
  layout: ReturnType<typeof scalarLayout>,
  ...
  let plain =
    !startsWithDocumentSeparator(string) &&
  ...
  switch (style) {
    ...
    case STYLE_FOLDED:
      return (
        ...
        blockHeader(string, blockIndent) +
        dropEndingNewline(
          indentString(foldBlockScalar(string, lineWidth), indent),
        )
      );
    case STYLE_DOUBLE:
...
function writeBlockMapping(
  ...
  for (let index = 0, length = items.length; index < length; index += 1) {
    ...
    const keyHasLineBreak =
      key.kind === "scalar" && key.value.indexOf("\n") !== -1;
    const explicitPair = keyIsBlock || keyHasLineBreak || keyText.length > 1024;

    if (explicitPair) {
      if (CHAR_LINE_FEED === keyText?.charCodeAt(0)) {
        pairBuffer += "?";
      } else {
        pairBuffer += "? ";
      }
    ...
    const keyColonSep =
      !explicitPair && (key.kind === "alias" || keyIsBareProps) ? " " : "";
...
function writeNode(
  ...
  if (node.kind === "mapping") {
    ...
    shouldPrintTag =
      node.style.tagged ||
      (style !== STYLE_PLAIN && node.tag !== state.defaultScalarTagName);
  }
...
function rootStartsOwnLine(node: Node) {
  return (
    (node.kind === "sequence" || node.kind === "mapping") &&
    !node.style.flow &&
    node.items.length !== 0 &&
    !node.style.tagged &&
    node.anchor === undefined
...
function isOpenEnded(node: Node) {
  ...
  let leaf = node;
  while (
    (leaf.kind === "sequence" || leaf.kind === "mapping") &&
    !leaf.style.flow &&
    leaf.items.length !== 0
  ) {
    leaf =
...
function present(documents: Document[], options: PresenterOptions): string {
  ...
  for (let index = 0; index < documents.length; index += 1) {
    ...
    const marker =
      doc.explicitStart || hasDirectives || (index > 0 && !previousEnded);
    ...
    if (doc.contents === null) {
      ...
      const sep =
        body === ""
          ? ""
          : hasDirectives || rootStartsOwnLine(doc.contents)
            ? "\n"
            : " ";
      result += `---${sep}${body}\n`;
    } else {
      result +=
        writeNode(state, 0, doc.contents, { block: true, compact: true }) +
        "\n";
    }

    previousEnded =
      doc.explicitEnd || (doc.contents !== null && isOpenEnded(doc.contents));
    if (previousEnded) {
      result += "...\n";
    }
  }
...
export { DEFAULT_PRESENTER_OPTIONS, present, type PresenterOptions };
