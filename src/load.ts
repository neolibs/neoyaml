...
interface LoadOptions
  extends ParserOptions, Omit<ConstructorOptions, "source"> {}
...
function loadDocuments(input: string, options: LoadOptions = {}) {
  ...
  const PARSER_OPT_KEYS = Object.keys(
    DEFAULT_PARSER_OPTIONS,
  ) as (keyof typeof DEFAULT_PARSER_OPTIONS)[];
  const CONSTRUCTOR_OPT_KEYS = Object.keys(
    DEFAULT_CONSTRUCTOR_OPTIONS,
  ) as (keyof typeof DEFAULT_CONSTRUCTOR_OPTIONS)[];

  const events = parseEvents(source, pick(opts, PARSER_OPT_KEYS));
  return constructFromEvents(events, {
...
function loadAll(
  ...
  if (typeof iteratorOrOptions === "function") {
    ...
    iteratorOrOptions !== null &&
    typeof iteratorOrOptions === "object"
  ) {
...
export { load, loadAll, type LoadOptions };
