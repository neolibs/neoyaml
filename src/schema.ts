...
class Schema {
  ...
  constructor(tags: readonly TagDefinition[]) {
    ...
    const implicitScalarAnyFirstChar = implicitScalarTags.filter(
      (tag) => tag.implicitFirstChars === null,
    );
    ...
    for (const key of keys) {
      implicitScalarByFirstChar.set(
        ...
        implicitScalarTags.filter(
          (tag) =>
            tag.implicitFirstChars === null ||
            tag.implicitFirstChars.indexOf(key) !== -1,
        ),
      );
    }
...
const FAILSAFE_SCHEMA: Schema = new Schema([strTag, seqTag, mapTag]);
...
const YAML11_SCHEMA: Schema = new Schema([
  ...FAILSAFE_SCHEMA.tags,
  nullYaml11Tag,
  boolYaml11Tag,
  intYaml11Tag,
