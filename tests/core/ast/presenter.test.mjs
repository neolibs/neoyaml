...
function presentParsed(input) {
  ...
  return present(eventsToAst(events, { source: input, schema: CORE_SCHEMA }), {
    schema: CORE_SCHEMA,
  });
}
...
describe("ast presenter", () => {
  it("keeps quoteFlowKeys outside an explicit long flow key", () => {
    ...
    const output = present(documents, {
      schema: CORE_SCHEMA,
      quoteFlowKeys: true,
    });
  ...
  it("emits a bare marker for an explicit-start null document", () => {
    const output = present(
      [{ contents: null, directives: [], explicitStart: true }],
      { schema: CORE_SCHEMA },
    );
  ...
  it("keeps explicit parsed tag spelling", () => {
    ...
    assert.equal(
      presentParsed("!<tag:yaml.org,2002:str> 123\n"),
  ...
  it("applies flow recursively to descendants", () => {
    ...
    assert.equal(
      present(documents, { schema: CORE_SCHEMA }),
      '[{a: [1, 2], b: "x\\ny"}]\n',
    );
  });
  ...
  it("never folds a tab-indented line over the width limit", () => {
    ...
    assert.deepEqual(load(source, { schema: CORE_SCHEMA }), {
      k: `${longTab}\ntail\n`,
    });
    assert.equal(presentParsed(source), source);
  });
  ...
  it("preserves block scalar values through an AST round-trip", () => {
    ...
    sweep = sweep.concat(
      sweep.map((body) => body.replaceAll("a", "a".repeat(90))),
    );
    ...
    for (const body of sweep) {
      for (const header of ["|", "|-", "|+", ">", ">-", ">+"]) {
        ...
        checked++;
        assert.deepEqual(
          load(presentParsed(source), { schema: CORE_SCHEMA }),
          value,
  ...
  it("propagates seqNoIndent to nested sequences", () => {
    ...
    assert.equal(
      present(documents, { schema: CORE_SCHEMA, seqNoIndent: true }),
      "- items:\n  - a: 1\n",
    );
  });
});
