...
import { visit, VISIT_BREAK, VISIT_SKIP } from "neoyaml";
...
const map = (...pairs) => ({
  kind: "mapping",
  items: pairs.map(([key, value]) => ({ key, value })),
});
...
describe("visit", () => {
  describe("traversal order", () => {
    ...
    it("visits each mapping key before its value", () => {
      const documents = [
        doc(map([scalar("k1"), scalar("v1")], [scalar("k2"), scalar("v2")])),
      ];
    ...
    it("descends depth-first (pre-order) through nested containers", () => {
      ...
      assert.deepEqual(collect(documents), [
        "mapping",
        "a",
        "sequence",
        "b",
  ...
  describe("visit context", () => {
    ...
    it("reports depth, parent and isKey for sequence items", () => {
      ...
      const ctxByLabel = {};
      visit([doc(root)], (node, ctx) => {
        ctxByLabel[labelOf(node)] = ctx;
      });
      assert.deepEqual(ctxByLabel.sequence, {
        depth: 0,
        parent: null,
        isKey: false,
      });
    ...
    it("increments depth per nesting level", () => {
      ...
      const depthByLabel = {};
      visit([doc(root)], (node, ctx) => {
        depthByLabel[labelOf(node)] = ctx.depth;
      });
  ...
  describe("VISIT_SKIP", () => {
    it("does not descend into a skipped container but continues with siblings", () => {
      ...
      const seen = collect(documents, (node) =>
        node === skipped ? VISIT_SKIP : undefined,
      );
    ...
    it("still visits the skipped node itself", () => {
      ...
      const seen = collect([doc(root)], (node) =>
        node === root ? VISIT_SKIP : undefined,
      );
    ...
    it("is a no-op on a leaf", () => {
      ...
      const seen = collect(documents, (node) =>
        node.kind === "scalar" ? VISIT_SKIP : undefined,
      );
  ...
  describe("VISIT_BREAK", () => {
    it("stops the whole traversal immediately", () => {
      ...
      const seen = collect(documents, (node) =>
        node.value === "stop" ? VISIT_BREAK : undefined,
      );
    ...
    it("breaking on a mapping key stops before the value", () => {
      ...
      const seen = collect(documents, (node) =>
        node.value === "k" ? VISIT_BREAK : undefined,
      );
