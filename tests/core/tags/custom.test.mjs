...
const schema = CORE_SCHEMA.withTags([
  ...
  defineMappingTag("!foo", {
    ...
    addPair: (container, key, value) => {
      ...
      else if (key === "my-another-parameter")
        container.myAnotherParameter = value;
    },
    has: () => false,
    keys: (container) => Object.keys(container),
    get: (container, key) => container[key],
...
`;
    ...
    for (let i = 0; i < expected.length; i++) {
      ...
      assert.strictEqual(
        Object.getPrototypeOf(actual[i]),
        Object.getPrototypeOf(expected[i]),
      );
    }
  });
  ...
  it("custom tag with multiple node kinds", () => {
    ...
    assert.deepStrictEqual(
      load("!Include foobar", { schema: multiSchema }),
      "foobar",
    );
    assert.deepStrictEqual(
      load("!Include\n  location: foobar", { schema: multiSchema }),
      { location: "foobar" },
    );
  });
  ...
  it("dumps a prefix-matched tag with a dynamic name", () => {
    const dynamicSchema = CORE_SCHEMA.withTags(
      defineScalarTag("!", {
        matchByTagPrefix: true,
  ...
  it("finalizes a sequence carrier into an immutable value", () => {
    ...
    const value = load("{ original: &a !immutable [one, two], alias: *a }", {
    ...
    assert.deepStrictEqual(
      ...
      new ImmutableSequence([]),
    );
    assert.equal(
      dump(value.original, { schema: immutableSchema }),
      "!immutable\n- one\n- two\n",
    );
  });
  ...
  it("finalizes a mapping carrier before exposing it as a value", () => {
    ...
    const immutableSchema = CORE_SCHEMA.withTags(
      defineMappingTag("!immutable", {
        create: () => new Map(),
    ...
    const value = load("!immutable { one: 1, two: 2 }", {
      schema: immutableSchema,
    });
    ...
    assert.equal(
      dump(value, { schema: immutableSchema }),
      "!immutable\none: 1\ntwo: 2\n",
    );
  });
  ...
  it("reports finalize errors at the collection start", () => {
    const immutableSchema = CORE_SCHEMA.withTags(
      defineSequenceTag("!immutable", {
        ...
        finalize: () => {
          throw new Error("cannot create immutable value");
        },
        identify: Array.isArray,
        represent: (value) => value,
      }),
  ...
  it("reports non-Error values thrown by finalize", () => {
    const immutableSchema = CORE_SCHEMA.withTags(
      defineSequenceTag("!immutable", {
        ...
        addItem: () => {},
        finalize: () => {
          throw "cannot create immutable value";
    ...
    assert.throws(
      ...
      (error) =>
        ...
        error.reason === "cannot create immutable value",
    );
  });
  ...
  it("does not call the placeholder finalizer when the carrier is the result", () => {
    ...
    tag.finalize = () => {
      throw new Error("placeholder finalizer was called");
    };

    assert.deepStrictEqual(
      load("!identity [one]", { schema: CORE_SCHEMA.withTags(tag) }),
      ["one"],
    );
  });
});
