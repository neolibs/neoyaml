...
describe("load options", () => {
  ...
  it("maxTotalMergeKeys — caps total merge keys", () => {
    ...
    assert.doesNotThrow(() =>
      load(merge(3), { schema: YAML11_SCHEMA, maxTotalMergeKeys: 5 }),
    );
    assert.throws(
      () => load(merge(3), { schema: YAML11_SCHEMA, maxTotalMergeKeys: 2 }),
      /maxTotalMergeKeys/,
    );
    assert.doesNotThrow(() =>
      load(merge(3), { schema: YAML11_SCHEMA, maxTotalMergeKeys: -1 }),
    );

    const result = load(createMergeChain(150), {
...
`;
    assert.doesNotThrow(() =>
      loadAll(src, { schema: YAML11_SCHEMA, maxTotalMergeKeys: 4 }),
    );
    assert.throws(
      () => loadAll(src, { schema: YAML11_SCHEMA, maxTotalMergeKeys: 3 }),
      /maxTotalMergeKeys/,
    );
  });
...
`;
  ...
  it("loadAll — options reach every argument form", () => {
    ...
    const collected = [];
    assert.equal(
      loadAll(src, (doc) => collected.push(doc), { json: true }),
      undefined,
    );
