...
describe("dump options", () => {
  ...
  it("sortKeys — orders mapping keys", () => {
    ...
    assert.equal(
      dump({ b: 1, a: 2 }, { sortKeys: (x, y) => y.localeCompare(x) }),
      "b: 1\na: 2\n",
    );
  });
  ...
  it("forceQuotes — quotes non-key strings using quoteStyle", () => {
    assert.equal(
      dump({ hello: "world" }, { forceQuotes: true }),
      "hello: 'world'\n",
    );
    assert.equal(
  ...
  it("flowLevel — quotes a colon followed by a flow indicator", () => {
    ...
    for (const value of [":{", ":[", ":,", ":}", ":]", "x:{"]) {
      ...
      assert.deepStrictEqual(load(dump({ k: value }, { flowLevel: 0 })), {
        k: value,
      });
    }
  });
  ...
  it("flowBracketPadding — pads inside flow brackets", () => {
    ...
    assert.equal(
      dump([1, 2], { flowLevel: 0, flowBracketPadding: true }),
      "[ 1, 2 ]\n",
    );
    assert.equal(
      dump({ a: 1, b: 2 }, { flowLevel: 0, flowBracketPadding: true }),
      "{ a: 1, b: 2 }\n",
    );
  });
  ...
  it("flowSkipCommaSpace — drops the space after flow commas", () => {
    assert.equal(dump([1, 2], { flowLevel: 0 }), "[1, 2]\n");
    assert.equal(
      dump([1, 2], { flowLevel: 0, flowSkipCommaSpace: true }),
      "[1,2]\n",
    );
    assert.equal(
      dump({ a: 1, b: 2 }, { flowLevel: 0, flowSkipCommaSpace: true }),
      "{a: 1,b: 2}\n",
    );
  });
  ...
  it("flowSkipColonSpace — drops the space after flow colons", () => {
    ...
    assert.equal(
      dump({ a: 1 }, { flowLevel: 0, flowSkipColonSpace: true }),
      "{a:1}\n",
    );
  });
  ...
  it("quoteFlowKeys — quotes keys in flow mappings", () => {
    assert.equal(dump({ a: 1 }, { flowLevel: 0 }), "{a: 1}\n");
    assert.equal(
      dump({ a: 1 }, { flowLevel: 0, quoteFlowKeys: true }),
      '{"a": 1}\n',
    );
  });
  ...
  it("tagBeforeAnchor — emits the tag before the anchor", () => {
    ...
    assert.equal(
      dump([shared, shared], { schema }),
      "- &ref_0 !t\n  x: 1\n- *ref_0\n",
    );
    assert.equal(
      dump([shared, shared], { schema, tagBeforeAnchor: true }),
      "- !t &ref_0\n  x: 1\n- *ref_0\n",
    );
  });
});
