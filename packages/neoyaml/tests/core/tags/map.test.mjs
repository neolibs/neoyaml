...
const variants = [
  [
    "PLAIN_OBJ_MAP",
    CORE_SCHEMA.withTags(mergeTag),
    (entries) => Object.fromEntries(entries),
  ],
  [
    "REAL_MAP",
    CORE_SCHEMA.withTags(realMapTag, mergeTag),
    (entries) => new Map(entries),
  ],
  [
    "LEGACY_OBJ_MAP",
    CORE_SCHEMA.withTags(legacyMapTag, mergeTag),
    (entries) => Object.fromEntries(entries),
  ],
];
...
`;
  ...
  describe("tags/map/plain object map", () => {
    it("defines __proto__ as an own data property", () => {
      const result = load("{ __proto__: { polluted: true } }", {
        schema: CORE_SCHEMA,
      });
