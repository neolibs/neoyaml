...
function tagNameFull(
  rawTag: string,
  tagHandlers?: Readonly<Record<string, string>>,
): string {
  ...
  const prefix =
    tagHandlers?.[handle] ?? DEFAULT_TAG_HANDLERS[handle] ?? handle;

  return (
    decodeURIComponent(prefix) + decodeURIComponent(rawTag.slice(handle.length))
  );
}

function tagNameShort(fullTag: string): string {
  let tag = fullTag;
...
export { tagNameFull, tagNameShort };
