...
const DEFAULT_CONSTRUCTOR_OPTIONS: Required<
  Omit<ConstructorOptions, "source">
> = {
  filename: "",
  schema: CORE_SCHEMA,
  json: false,
...
function eventPosition(event: Event) {
  ...
  if ("anchorStart" in event && event.anchorStart !== NO_RANGE)
    return event.anchorStart;
  if ("valueStart" in event && event.valueStart !== NO_RANGE)
...
function lookupTag<
  T extends ScalarTagDefinition | SequenceTagDefinition | MappingTagDefinition,
>(
...
function collectionTag<
  Tag extends SequenceTagDefinition | MappingTagDefinition,
>(
  state: ConstructorState,
...
function mergeKeys(
  ...
  frame: MappingFrame,
  source: unknown,
  sourceTag: MappingTagDefinition<any, any>,
) {
