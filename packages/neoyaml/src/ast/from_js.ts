...
function buildRepresentTypes(schema: Schema): RepresentType[] {
  ...
  const explicitTags = schema.tags.filter(
    (t) => !(t.nodeKind === "scalar" && t.implicit) && !defaultTags.has(t),
  );
...
function matchTag(
  state: FromJsState,
  object: unknown,
): { tag: TagDefinition; tagName: string; implicitTag: boolean } | null {
  for (
...
export { jsToAst, type FromJsOptions };
