...
function implicitScalarTagName(state: FromEventsState, source: string) {
  const { schema } = state;
  const candidates =
    schema.implicitScalarByFirstChar.get(source.charAt(0)) ??
    schema.implicitScalarAnyFirstChar;
  for (const tag of candidates) {
    if (tag.resolve(source, false, tag.tagName) !== NOT_RESOLVED)
      return tag.tagName;
  }
...
export { eventsToAst, type FromEventsOptions };
